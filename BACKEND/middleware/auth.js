...
const protectHead = async (req, res, next) => {
  let token;
  ...
  if (req.headers.authorization && req.headers.authorization.startsWith('Bearer')) {
    try {
      ...
      req.head = await Head.findById(decoded.id).select('-password');
      if (!req.head) {
        return res.status(401).json({ success: false, message: 'Not authorized, head not found' });
      }
...
module.exports = { protect, protectFaculty, protectHead };
