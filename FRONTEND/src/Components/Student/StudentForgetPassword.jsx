...
const StudentForgetPassword = () => {
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      ...
      if (error.response?.data?.message === 'Invalid OTP' || error.response?.data?.message === 'OTP has expired') {
        setStep(2); 
      }
    } finally {
      setLoading(false);
        ...
        {step === 1 && (
          <form onSubmit={handleSendOtp} className="space-y-6">
            <div className="space-y-1">
              ...
              <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all bg-white/50" placeholder="john@example.com" />
            </div>
            ...
            <p className="text-center text-sm text-slate-600 mt-4">
              Remembered your password? <Link to="/student/login" className="text-indigo-600 font-semibold hover:underline">Login here</Link>
            </p>
          </form>
        )}
        ...
        {step === 2 && (
          <form onSubmit={handleVerifyOtp} className="space-y-6">
            <div className="space-y-1">
              ...
              <input required type="text" maxLength={6} value={otp} onChange={(e) => setOtp(e.target.value.replace(/[^a-zA-Z0-9]/g, '').toUpperCase())} className="w-full text-center tracking-widest font-mono text-2xl px-4 py-3 rounded-xl border border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all bg-white/50 uppercase" placeholder="XXXXXX" />
            </div>

            <button type="submit" className="w-full py-3.5 px-4 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white rounded-xl font-semibold shadow-lg shadow-indigo-200 transition-all transform hover:-translate-y-0.5 mt-4">
              Continue
            </button>
            <button type="button" onClick={() => setStep(1)} className="w-full py-2 text-slate-500 hover:text-indigo-600 text-sm font-medium transition-colors">
