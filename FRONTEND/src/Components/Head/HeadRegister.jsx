...
const HeadRegister = () => {
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await axios.post(`${import.meta.env.VITE_BACKEND_URL || 'https://campus-bridge-tau.vercel.app'}/api/head/register`, submitData, {
  ...
  const handleOtpSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await axios.post(`${import.meta.env.VITE_BACKEND_URL || 'https://campus-bridge-tau.vercel.app'}/api/head/verify-otp`, {
        email: formData.email,
        otp
      });

      if (res.data.success) {
        toast.success(`Account verified! Your Head ID is: ${res.data.headId}`);
        navigate('/head/login');
      }
    } catch (error) {
      ...
      <div className="w-full max-w-2xl bg-white/70 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-white/50">
        ...
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold bg-gradient-to-r from-red-600 to-rose-600 bg-clip-text text-transparent">
            {step === 1 ? 'Head Registration' : 'Verify Your Email'}
          </h2>
          <p className="text-slate-500 mt-2">
            {step === 1 ? 'Join CampusBridge as a Sector Head / HOD.' : `We've sent a 6-character OTP to ${formData.email}`}
          </p>
        </div>
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl mb-6">
          ...
          <button type="button" className="flex-1 py-2 font-semibold text-sm rounded-lg bg-white text-red-700 shadow-sm transition-all" disabled>Head / HOD</button>
        </div>
          ...
          <form onSubmit={handleRegisterSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="space-y-1">
                ...
                <input required type="email" name="email" value={formData.email} onChange={handleInputChange} className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-red-500 focus:ring-2 focus:ring-red-200 outline-none transition-all bg-white/50" placeholder="jane.hod@college.edu" />
              </div>
            ...
            <button disabled={loading || !isMobileValid || !isPasswordValid || !doPasswordsMatch} type="submit" className="w-full py-3.5 px-4 bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white rounded-xl font-semibold shadow-lg shadow-red-200 transition-all transform hover:-translate-y-0.5 mt-6 disabled:opacity-50 disabled:cursor-not-allowed">
              {loading ? 'Processing...' : 'Register Head Account'}
            </button>
            
            <p className="text-center text-sm text-slate-600 mt-4">
              Already have a head account? <Link to="/head/login" className="text-red-600 font-semibold hover:underline">Login here</Link>
            </p>
